  ...
  $.getJSON("projects.json", function(data) {
      ...
      var content = data.projects.map(function (project) {
          ...
          if (project.code_url.length > 1) {
              codeUrl = "<a target='_blank' href='" + $("<td>").text(project.code_url).html() + "'>Näytä lähdekoodi &raquo;</a>";
          }

          if (project.service_url.length > 1) {
              serviceUrl = "<a target='_blank' href='" + $("<td>").text(project.service_url).html() + "'>Siirry palveluun &raquo;</a>";
          }
